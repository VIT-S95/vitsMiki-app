...
while (!feof($file)) {
    ...
    // USERMETA - capabilities (identifier les clients)
    ...
    // USERMETA - vits_company (nom société)
    ...
    // USERMETA - vits_id (numéro client Kizeo ex: C0476)
    ...
    // USERMETA - nickname (email signataire)
    ...
    // POSTMETA
    $relevantKeys = ["'client'","'date_de_debut'","'date_de_fin'","'duree_contrat'","'nombre_heure_allouees'","'periodes'","'contrat_id_vits'","'contrat_de_maintenance'","'date'","'forfait_temps'","'forfait'"];
...
// Nettoyage complet en désactivant les foreign keys SQLite
echo "Nettoyage...\n";
...
echo "Import clients...\n";
$c = 0;
$dejaVus = []; // pour éviter les doublons vits_id dans le même import
foreach ($clientIds as $uid) {
    $meta    = $usermeta[$uid] ?? [];
    $user    = $users[$uid] ?? [];
    $numero  = $meta['vits_id'] ?? '';
    if (empty(trim($numero))) continue; // skip sans numéro Kizeo
    if (isset($dejaVus[$numero])) continue; // skip doublons
    $dejaVus[$numero] = true;
...
foreach ($interventionIds as $pid) {
    ...
    if (!$date) continue;
    // Numero Kizeo depuis le titre ex: "Intervention n°21727358"
