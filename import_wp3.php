...
while (!feof($file)) {
    ...
    // USERMETA - nom_societe
    if (str_contains($line, "'nom_societe'")) {
        preg_match_all("/\(\d+,\t(\d+),\t'nom_societe',\t'([^']*)'\)/", $line, $um);
        foreach ($um[1] as $i => $uid) {
    ...
    if (str_contains($line, "'display_name'")) {
        preg_match_all("/\(\d+,\t(\d+),\t'display_name',\t'([^']*)'\)/", $line, $um);
        foreach ($um[1] as $i => $uid) {
            if (!isset($usermeta[$uid])) $usermeta[$uid] = [];
...
Client::whereNotNull('numero_client_kizeo')
    ->where(function($q){ $q->whereNull('nom_societe')->orWhere('nom_societe',''); })
    ->delete();
